...
public class LottoNumTraining {
	...
	public static void main(String[] args) {
		...
		// 1. 6개의 1~45까지의 랜덤 수 발생
		// 2. 각종 조건들 추가 
		// 2-1. 6개 내에서 중복 숫자 안됨
		// 2-2. 3연속 숫자 안됨
		// 2-3. 현재까지 나온 번호와 중복되지않아야함
		// 3. 조건에 걸릴시 재실행 
		// 4. 출력 
...
//		}
		...
		for(int i=0; i<lottoNum.length-1;i++) {				// lottoNum 0~4 
			for(int j =i+1; j<lottoNum.length;j++) {			// lottoNum 1~5 비교 
				if(lottoNum[i]==lottoNum[j]) {				// 같은게 있으면 다시 뽑아
					for(int l=0;l<lottoNum.length;l++) {
						System.out.print(lottoNum[l]+" ");
					}
					for(int k=0;k<lottoNum.length;k++) {
						lottoNum[k]= (int)(Math.random()*44+1);
					}
					System.out.println("중복된 숫자가 있습니다. 다시뽑았습니다");
					IsSame=true;
				} 
			}
		}
		...
		// 2-3 현재까지 나온 번호와 중복되지않아야함 
		// 2-3-1 현재까지 나온 번호를 어딘가에 입력 하고 나서 비교
		
//		int[][] lottoNumHistory = new int[1028][6]; //1028회차 6개의 숫자(이미 오름차순 정렬되어있음)
		
		
		int[][] lottoNumHistory = {{5,7,12,13,18,35},
				{14,16,27,35,39,45},
				{5,12,13,31,32,41},
				{8,9,20,25,29,33},
				{9,18,20,22,38,44},
...
//		for(int i=0; i<lottoNumHistory.length; i++) {
//			if(9 ==lottoNumHistory[i][0] && 13 ==lottoNumHistory[i][1] && 21 ==lottoNumHistory[i][2] && 25 ==lottoNumHistory[i][3] && 32 ==lottoNumHistory[i][4] && 42 ==lottoNumHistory[i][5]) {
//				System.out.println("과거내역에 동일한 번호가 있었습니다.");
//				IsHistory=true;
//			}
//		}
		//---------------------- 테스트 9 13 21 25 32 42 true 테스트 9 13 21 25 33 42 false
		
		
		// 다시 뽑기 조건 합체
		System.out.println("본편");
...
//			}
//		}
//		System.out.println(lottoNumHistory[1027][4]);
//		
//		
//		
		}
